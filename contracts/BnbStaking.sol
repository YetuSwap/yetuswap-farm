...
import './yetuswap-lib/math/SafeMath.sol';
import './yetuswap-lib/token/BEP20/IBEP20.sol';
import './yetuswap-lib/token/BEP20/SafeBEP20.sol';
import './yetuswap-lib/access/Ownable.sol';
...
contract BnbStaking is Ownable {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 yetuReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
