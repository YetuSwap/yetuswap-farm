...
contract('MansaMusa', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    ...
    this.musa = await MansaMusa.new(
      this.yetu.address,
      this.afrikan.address,
      dev,
      '10',
      '10',
      { from: minter }
  ...
  it('real case', async () => {
    ...
    await this.musa.add('500', this.lp4.address, true, { from: minter });
    ...
    assert.equal(
      (await this.yetu.balanceOf(this.lottery.address)).toString(),
      '0'
    );
    ...
    assert.equal((await this.yetu.balanceOf(carol)).toString(), '5');
  });
  ...
  it('setReceiver', async () => {
    ...
    assert.equal((await this.yetu.balanceOf(carol)).toString(), '7');
    await this.lottery.setReceiver(alice, { from: dev });
    assert.equal((await this.lottery.pendingReward('1')).toString(), '7');
    await this.lottery.harvest(1, { from: dev });
    assert.equal((await this.yetu.balanceOf(alice)).toString(), '15');
  });
